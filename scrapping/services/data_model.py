...
class DataModel(Model):
    ...
    document_hash = UnicodeAttribute(hash_key=True)
    ...
    def __str__(self):
        return 'document_hash:{}, state:{}'.format(self.document_hash, self.state)

    def get_key(self):
        return u'{}/{}'.format(KEY_BASE, self.document_hash)
    ...
    def save(self, conditional_operator=None, **expected_values):
        try:
            ...
            logger.error('save {} failed: {}'.format(self.document_hash, e), exc_info=True)
            raise e
        ...
        if self.state != State.UPLOADED.name:
            raise AssertionError(
                'Asset {} is marked as {}, must be marked {} to retrieve.'.format(
                    self.document_hash, self.state, State.UPLOADED.name
                )
            )
    ...
    def mark_received(self):
        ...
        logger.debug('mark asset received: {}'.format(self.document_hash))
        self.save()
    ...
    def mark_uploaded(self):
        ...
        logger.debug('mark asset uploaded: {}'.format(self.document_hash))
        self.save()
    ...
    def mark_deleted(self):
        ...
        logger.debug('mark asset deleted: {}'.format(self.document_hash))
        self.save()