...
def payload_creation(page_content):
    page_content = re.sub(
        r'(<div[^>]*?class=\"[^>]*?debate-item[^>]*?>)', r'<block1><block2>\1', page_content)
    page_content = re.sub(r'<p\s*class=\"hs_para\">\s*</p>', '', page_content)

    time_stamp = r'<div[^>]*?class=\"timestamp\"[^>]*?>\s*<time\s*datetime=\"([0-9\/]+)\s+([0-9\:]+)\">'

    title = clean(re.findall(
        r'<h1[^>]*?>([\w\W]*?)</h1>', page_content, re.IGNORECASE)[0])
    ...
    if volume:
        source_content = source_content + \
            '<div class="volume">' + str(volume[0]) + "</div>"
    ...
    if start_year:
        ...
        source_content = source_content + \
            '<div class="StartYear">' + str(start_year) + '</div>'
        source_content = source_content + \
            '<div class="EndYear">' + str(start_year + 1) + '</div>'
    ...
    for each_member_block in re.findall(
        ...
        time_check_list = re.findall(
            time_stamp, each_member_block, re.IGNORECASE)
        ...
        if column_number:
            try:
                ...
                print(e)
                block_column = column_number_list[-1] if len(
                    column_number_list) > 0 else ''
            column_number_list.append(block_column)

        if time_list:
            source_content = source_content + '<div class="Time"><p>' + \
                str(clean(time_list[0])) + "</p></div>"
        elif member_list:
            source_content = source_content + '<div class="AnswerText">'
            if block_time:
                source_content = source_content + '<div class="BlockTime">' + \
                    str(date_format_1(block_time)) + '</div>'
            if block_column:
                source_content = source_content + '<div class="BlockColumn">' + \
                    str(clean(block_column)) + '</div>'

            source_content = source_content + '<h3>' + \
                str(clean(member_list[0])) + '</h3>'
