...
const DummyWrapper = (props: ModalProps) => {
  const [isModalOpen, setIsModalOpen] = useState<boolean>(false);
  return (
    <>
      <button onClick={() => setIsModalOpen(true)}>open modal</button>
      {isModalOpen && (
        <Modal {...{ ...props, onClose: () => setIsModalOpen(false) }}>
          <p>
            ...
            rationally encounter consequences that are extremely painful. pains.
          </p>
        </Modal>
      )}
    </>
...
const Template: Story<ModalProps> = (args) => <DummyWrapper {...args} />;
...
Default.args = {
  ...
  title: 'Some title',
  buttons: [
    { label: 'Test button', type: 'primary' },
