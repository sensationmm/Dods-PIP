...
export const removeTeamMember: AsyncLambdaMiddleware<RemoveTeamMemberParameters> =
    async ({ userId, clientAccountId }) => {
        const clientAccount =
            await ClientAccountRepositoryV2.defaultInstance.findOne({
                uuid: clientAccountId,
            });

        const user = await UserProfileRepository.defaultInstance.findOne({
            uuid: userId,
        });
        ...
        if (clientAccountTeam.teamMemberType === TeamMemberTypes.ClientUser) {
            await IamRepository.defaultInstance.destroyUser({
                email: user.primaryEmail,
            });

            await UserProfileRepository.defaultInstance.updateUser(
                { isActive: false },
                { id: user.id }
            );
        }
