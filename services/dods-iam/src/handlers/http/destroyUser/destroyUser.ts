...
import { DestroyUserParameters } from "../../../domain";
import { AwsCognito } from "../../../services";

export const destroyUser: AsyncLambdaMiddleware<DestroyUserParameters> = async ({ email }) => {
    ...
    try {
        const result = await AwsCognito.defaultInstance.destroyUser(email);
